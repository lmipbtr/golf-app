...
score_record = filedialog.askopenfilename(title="Select Input Score History file",filetypes=(("xlsx files", "*.xlsx"),("all files", "*.*")))
filepath=score_record.split("/")[:-1]
x=0
...
course_list = filedialog.askopenfilename(title="Select Courses file",filetypes=(("xlsx files", "*.xlsx"),("all files", "*.*")))
filepath1=course_list.split("/")[:-1]
x=0
for i in range(len(filepath1)):
    filepath1.insert(i+x,"/")
    x+=1
filepath1.append("/")
filepath1.pop(0)
filepath1="".join(filepath1)

df_score_history = pd.read_excel(score_record, sheet_name='Score')
df_score_history['Date'] = pd.to_datetime(df_score_history['Date'],format='%d/%m/%Y')

df_artificial_rounds = pd.read_excel(score_record, sheet_name='Artificial Rounds')
df_artificial_rounds['Date'] = pd.to_datetime(df_artificial_rounds['Date'],format='%d/%m/%Y')

df_course_ratings = pd.read_excel(course_list, sheet_name='Ratings')
df_course_par_index = pd.read_excel(course_list,sheet_name='Par_Index')
...
graph = plt.plot(df_round_summary['New GA Handicap'])
...
results_file=filepath+"Handicap Results.txt"
